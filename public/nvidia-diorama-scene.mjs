...
var M = {
  ...
  cyanLed: mat(C.cyan, { emissive: C.cyan, emissiveIntensity: 1, roughness: 0.2 }),
...
function makeChipDesignTexture() {
  ...
  drawNvidiaEye(ctx, chipCx - 32, chipCy - 6, 1);
  ctx.fillStyle = C.nvGreen;
...
function texturedPlane(g, tex, x, y, z, w, h, emissive = 0, rotY = 0) {
  const m = new THREE.MeshStandardMaterial({
    map: tex,
    side: THREE.DoubleSide,
    roughness: 0.4,
    emissive: emissive > 0 ? "#ffffff" : "#000000",
    emissiveMap: emissive > 0 ? tex : void 0,
...
function addLedRim(g, cx, cy, cz, w, d, inset, intensity, name, color = C.nvGreen) {
...
function buildBase(g) {
  ...
  addLedRim(g, 0, 0.065, 0, 4.4, 3.2, 0.18, 1, "rim inner");
  const lampSpots = [
    [-1.9, 1.35],
    [1.9, 1.35],
    [-1.9, -1.35],
...
function buildHQ(g) {
  ...
  for (let col = 0; col < 5; col++) {
    box(grp, `mullion ${col}`, -0.58 + col * 0.29, 0.1, 0.662, 0.02, 0.52, 8e-3, M.dark);
  }
  ...
  const dishMount = cyl(grp, "dish mount", -0.52, 0.68, -0.42, 8e-3, 0.06, M.darkElev, 6);
  ...
  const dishLed = new THREE.Mesh(
    new THREE.SphereGeometry(8e-3, 6, 6),
...
function buildRTXHero(g) {
  ...
  box(grp, "rtx silver top", 0, 0.134, 0, 1.6, 8e-3, 0.55, M.silver);
  for (let i = 0; i < 24; i++) {
    const fz = -0.22 + i * 0.018;
    box(grp, `fin ${i}`, -0.35, 0, fz, 0.7, 0.2, 8e-3, M.fin);
  }
  ...
  fanGrp.add(fanRim);
  for (let b = 0; b < 11; b++) {
    const blade = new THREE.Mesh(new THREE.BoxGeometry(0.16, 5e-3, 0.04), M.fin);
  ...
  for (let i = 0; i < 8; i++) {
    box(grp, `bp vent ${i}`, -0.55 + i * 0.14, 0, -0.276, 0.1, 0.18, 6e-3, M.fin);
  }
...
function buildChipDisplay(g) {
  ...
  grp.rotation.x = -0.08;
  g.add(grp);
  box(grp, "display bezel", 0, 0, 0, 2, 1.3, 0.06, M.dark);
...
function buildAIInfra(g) {
  ...
  box(g, "infra cyan rim", bx, 0.122, bz + 0.42, 1.38, 8e-3, 0.012, M.cyanRim);
  box(g, "infra cyan rim b", bx, 0.122, bz - 0.42, 1.38, 8e-3, 0.012, M.cyanRim);
  const rackW = 0.28;
  const rackH = 0.78;
  const rackD = 0.55;
  const gap = 0.04;
  ...
  for (let r = 0; r < 4; r++) {
    ...
    for (let c = 0; c < 2; c++) {
      const cable = new THREE.Mesh(
        new THREE.CylinderGeometry(8e-3, 8e-3, 0.3, 6),
...
function createNvidia() {
  ...
  root.userData.tick = (time) => {
    ...
    u.hqWindows.forEach((w) => {
      w.material.emissiveIntensity = 0.55;
    });
    u.pulseLeds.forEach((led, i) => {
      led.material.emissiveIntensity = 1 + Math.sin(time * 1.5 + i * 0.7) * 0.15;
    });
    u.chipScreen.material.emissiveIntensity = 0.95;
  };
  return root;
}
