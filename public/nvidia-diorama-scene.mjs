...
var M = {
  ...
  nvGreenMid: mat(C.nvGreen, { emissive: C.nvGreen, emissiveIntensity: 1, roughness: 0.25 }),
...
function cyl(g, name, x, y, z, r, h, material, radial = 16) {
...
function drawNvidiaEye(ctx, cx, cy, scale) {
...
function texturedPlane(g, tex, x, y, z, w, h, emissive = 0.55, rotY = 0) {
...
function addLedRim(g, cx, cy, cz, w, d, inset, intensity, name) {
...
function buildBase(g) {
  ...
  addLedRim(g, 0, rimY, 0, 4.4, 3.2, 0.18, 1, "rim inner");
...
function buildHQ(g) {
  ...
  for (let col = 0; col < 5; col++) {
    box(grp, `mullion ${col}`, -0.52 + col * 0.26, 0.08, 0.612, 0.02, 0.5, 8e-3, M.dark);
  }
  ...
  cyl(grp, "antenna", 0.5, 0.72, -0.35, 5e-3, 0.25, M.metal, 6);
...
function buildRTXHero(g) {
  ...
  box(grp, "rtx side L", -0.705, 0, 0, 5e-3, 0.32, 0.5, mat(C.nvGreen, { emissive: C.nvGreen, emissiveIntensity: 1.1 }));
  box(grp, "rtx side R", 0.705, 0, 0, 5e-3, 0.32, 0.5, mat(C.nvGreen, { emissive: C.nvGreen, emissiveIntensity: 1.1 }));
  ...
  fanGrp.add(fanRim);
  for (let b = 0; b < 9; b++) {
    const blade = new THREE.Mesh(new THREE.BoxGeometry(0.15, 5e-3, 0.04), M.fin);
    blade.rotation.z = b / 9 * Math.PI * 2;
    blade.position.set(0.06, 0, 0);
    fanGrp.add(blade);
  }
  cyl(fanGrp, "fan hub", 0, 0, 0, 0.035, 6e-3, mat("#222222", { metalness: 0.5 }), 16);
...
function buildChipDisplay(g) {
  ...
  grp.rotation.x = -0.05;
  g.add(grp);
  box(grp, "display bezel", 0, 0, 0, 1.6, 1, 0.06, M.dark);
  ...
  grp.add(kb);
  box(grp, "kb led 0", 0.22, -0.51, 0.18, 0.012, 8e-3, 8e-3, M.nvGreen);
  box(grp, "kb led 1", 0.28, -0.51, 0.18, 0.012, 8e-3, 8e-3, M.nvGreen);
...
function buildAIInfra(g) {
  ...
  for (let r = 0; r < 4; r++) {
    ...
    box(rack, "rack door", 0, 0, rackD / 2 + 4e-3, rackW - 0.02, rackH - 0.04, 5e-3, M.dark);
    for (let row = 0; row < 9; row++) {
      ...
      if (row % 2 === 0) {
        box(rack, `rack stripe ${row}`, 0, led.position.y, rackD / 2 + 6e-3, rackW - 0.04, 6e-3, 3e-3, M.dark);
      }
    }
    box(rack, "status y", 0.08, rackH / 2 - 0.05, rackD / 2 + 0.01, 0.02, 0.02, 5e-3, mat("#ffcc00", { emissive: "#ffcc00", emissiveIntensity: 0.8 }));
    box(rack, "status r", -0.08, rackH / 2 - 0.12, rackD / 2 + 0.01, 0.02, 0.02, 5e-3, mat("#ff4444", { emissive: "#ff4444", emissiveIntensity: 0.8 }));
  }
  ...
  for (let i = 0; i < 3; i++) {
    const cable = new THREE.Mesh(
      new THREE.CylinderGeometry(8e-3, 8e-3, 0.35, 6),
...
function buildFrontLabels(g) {
  ...
  box(gpuGrp, "gpu label led", 0, 0.065, 0.051, 0.4, 6e-3, 5e-3, M.nvGreen);
  const nvTex = makeNvidiaLogoTexture("#1a1a1a");
  const nvGrp = new THREE.Group();
  nvGrp.position.set(1, 0.2, 1.35);
  g.add(nvGrp);
...
function createNvidia() {
  ...
  root.userData.tick = (time) => {
    ...
    u.pulseLeds.forEach((led, i) => {
      led.material.emissiveIntensity = 1.3 + Math.sin(time * 1.5 + i * 0.7) * 0.2;
    });
    u.chipScreen.material.emissiveIntensity = 0.85 + Math.sin(time * 0.4) * 0.05;
  };
  return root;
}
function createNvidiaFab() {
  return createNvidia();
}
export {
  createNvidia,
